...
try {
    ...
    Set-Content -LiteralPath (Join-Path $cleanRoot 'README.md') -Value @(
        ...
        'Identify the failing layer first; do not chase ACLs until it is known.'
    ) -Encoding UTF8
...
'@ -Encoding UTF8
    ...
    # windows-absolute-path: the well-known system locations this skill
    # documents (Git for Windows, the System32 WSL shim) should not be findings.
    $winPathSystemRoot = Join-Path $tempRoot 'winpath-system'
    New-Item -ItemType Directory -Path $winPathSystemRoot | Out-Null
    Set-Content -LiteralPath (Join-Path $winPathSystemRoot 'doc.md') -Value @'
Running C:\Program Files\Git\bin\bash.exe -lc inside the sandbox fails.
Running C:\Program Files\Git\usr\bin\bash.exe -lc fails with a signal pipe error.
Plain bash resolves to C:\Windows\System32\bash.exe (the WSL shim) instead.
'@ -Encoding UTF8
    $winPathSystemResult = Invoke-Scanner -ScanPath $winPathSystemRoot
    if ($winPathSystemResult.ExitCode -ne 0) {
...
Home: https://github.com/h8nc4y/codex-windows-sandbox-troubleshooting
...
'@ -Encoding UTF8
    ...
    if ($urlAllowedResult.ExitCode -ne 0) {
        Add-Failure "Expected allowlisted GitHub URL doc to pass, but scanner exited $($urlAllowedResult.ExitCode): $($urlAllowedResult.Output.Trim())"
    }

    # GitHub URL allowlist: any other repository URL is a finding.
    # Split the literal so this test file does not make the scanner flag itself.
    $urlDeniedRoot = Join-Path $tempRoot 'url-denied'
    New-Item -ItemType Directory -Path $urlDeniedRoot | Out-Null
    $deniedUrl = ('https://github' + '.com/') + 'example-org/example-repo'
    Set-Content -LiteralPath (Join-Path $urlDeniedRoot 'doc.md') -Value "See $deniedUrl for details." -Encoding UTF8
    $urlDeniedResult = Invoke-Scanner -ScanPath $urlDeniedRoot
    if ($urlDeniedResult.ExitCode -eq 0) {
        Add-Failure 'Expected non-allowlisted GitHub URL fixture to fail, but scanner exited 0.'
    }
    if ($urlDeniedResult.Output -notmatch 'non-allowlisted-github-repo-url') {
        Add-Failure "Expected output to name non-allowlisted-github-repo-url. Output: $($urlDeniedResult.Output.Trim())"
    }
